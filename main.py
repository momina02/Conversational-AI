...
import asyncio
from bot_logic import record_audio, transcribe_audio, generate_response, text_to_speech, play_audio
...
def start_call():
    try:
        while True:
            ...
            if not audio_file:
                return {"error": "🎙️ آڈیو ریکارڈ نہ ہو سکی"}
            ...
            if any(word in text.lower() for word in ["خدا", "الوداع"]):
                print("👋 کال ختم کی جا رہی ہے۔")
                break

            if not text.strip():
                print("⚠️ واضح اردو میں بات کریں۔")
                continue
            ...
            if not response.strip():
                print("⚠️ جواب موجود نہیں۔")
                continue

            # 🔊 Convert to Audio
            output_audio = asyncio.run(text_to_speech(response))
            if output_audio:
                play_audio(output_audio)
            else:
                print("⚠️ آڈیو تیار نہ ہو سکی۔")

        return {"message": "📞 کال ختم ہو گئی۔"}
    except Exception as e:
        return {"error": f"⚠️ خرابی: {e}"}
